...
import {
  GraduationCap, TrendingUp, ClipboardList, Calendar,
  BookOpen, Clock, Bell, Star, ChevronRight,
  UserCheck, CheckCircle2, AlertCircle,
} from 'lucide-react';

export function StudentDashboard() {
  const { currentUser } = useAppStore();
  const { data, isLoading, isError, error } = useStudentDashboard(currentUser?.email);
  ...
  const [studentInfo, setStudentInfo] = useState<{ className: string; rollNumber: string; name: string } | null>(null);

  useEffect(() => {
    if (currentUser?.email) {
      fetch('/api/students').then(r => r.json()).then((students: any[]) => {
        const s = students.find(st => st.email === currentUser.email) || students[0];
        if (s) setStudentInfo({ className: s.className, rollNumber: s.rollNumber, name: s.name });
      }).catch(() => {});
    }
  }, [currentUser?.email]);
  ...
  const firstName = studentInfo?.name?.split(' ')[0] || currentUser?.name?.split(' ')[0] || 'Student';
  ...
  const studentNotices = (data?.notices ?? []).filter(n => n.targetRole === 'all' || n.targetRole === 'student');
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-violet-600 via-purple-600 to-fuchsia-500 p-6 lg:p-8 text-white">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-violet-200 text-sm font-medium">{getGreeting()} 👋</span>
          </div>
          <h2 className="text-2xl lg:text-3xl font-bold">{firstName}!</h2>
          <p className="text-violet-200 mt-1">
            {studentInfo ? `${studentInfo.className} • Roll ${studentInfo.rollNumber}` : 'Welcome to Sigel School'}
          </p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          trend={avgGrade >= 75 ? 'Excellent' : avgGrade >= 60 ? 'Good' : 'Needs Work'}
          trendUp={avgGrade >= 60}
          color="emerald"
        />
        <StatCard
              ...
              <ScrollArea className="max-h-[320px]">
                <div className="space-y-2">
                  {todayTimetable.map((slot) => {
                    ...
                    const slotStart = new Date(now); slotStart.setHours(startH, startM);
                    const slotEnd = new Date(now); slotEnd.setHours(endH, endM);
                    ...
                    return (
                      ...
                      >
                        <div className="flex-shrink-0 w-20 text-center">
                          <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">{slot.startTime}</p>
                          <p className="text-xs text-gray-400 dark:text-gray-500">{slot.endTime}</p>
                        </div>
                        <div className={`w-0.5 h-10 rounded-full ${isCurrent ? 'bg-violet-400' : 'bg-gray-200 dark:bg-gray-700'}`} />
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-sm text-gray-900 dark:text-gray-100 truncate">{slot.subjectName}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">{slot.className}</p>
                        </div>
                        {isCurrent && (
                          <Badge className="bg-violet-500 text-white text-[10px] px-2">Live</Badge>
                        )}
                        {isPast && (
                  ...
                  return (
                    <div key={g.id} className="flex items-center justify-between">
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{g.subjectName}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">{g.examType}</p>
                      </div>
                      <div className="flex items-center gap-2 ml-2">
                        <Badge
                          ...
                          className={`text-xs font-semibold ${
                            ...
                            pct >= 60 ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400' :
                            'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400'
                          }`}
                        >
                          {g.grade || 'N/A'}
                        </Badge>
                        <span className="text-xs text-gray-500 dark:text-gray-400">{g.marks}/{g.maxMarks}</span>
                      </div>
                    </div>
                  );
                })}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <Badge
                          variant="secondary"
                          className={`text-[10px] ${
                            notice.priority === 'urgent' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' :
                            notice.priority === 'important' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400' :
                      ...
                      <h4 className="font-semibold text-sm text-gray-900 dark:text-gray-100">{notice.title}</h4>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">{notice.content}</p>
...
function StatCard({
  ...
  const colorMap: Record<string, string> = {
    ...
    emerald: 'from-emerald-500 to-teal-500 bg-emerald-50 dark:bg-emerald-900/30',
    ...
    <Card className="rounded-xl shadow-sm hover:shadow-md transition-shadow">
      <CardContent className="p-4">
        ...
        <div className="mt-2 flex items-center gap-1">
          <span className={`text-[10px] font-medium ${trendUp ? 'text-green-600' : 'text-amber-600'}`}>
            {trend}
          </span>
        </div>
      </CardContent>
