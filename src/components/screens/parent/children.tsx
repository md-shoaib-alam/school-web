...
import {
  GraduationCap, User, Calendar, Hash, BookOpen,
...
export function ParentChildren() {
  ...
  const getAttendanceForStudent = (studentId: string) => {
    ...
    if (records.length === 0) return { percentage: 0, present: 0, absent: 0, late: 0, total: 0 };
    const present = records.filter(a => a.status === 'present').length;
    const absent = records.filter(a => a.status === 'absent').length;
    const late = records.filter(a => a.status === 'late').length;
    return {
      percentage: Math.round(((present + late) / records.length) * 100),
      present, absent, late, total: records.length
    };
  };
  ...
  const getSubjectPerformance = (studentId: string) => {
    ...
    const subjectMap = new Map<string, { marks: number[]; maxMarks: number[]; grades: string[] }>();
    ...
    return Array.from(subjectMap.entries()).map(([subject, data]) => {
      const avgPct = data.marks.reduce((sum, m, i) => sum + (m / data.maxMarks[i]) * 100, 0) / data.marks.length;
      const bestGrade = data.grades.sort((a, b) => {
        const order = ['A+', 'A', 'B+', 'B', 'C', 'D'];
        return order.indexOf(a) - order.indexOf(b);
      })[0] || 'N/A';
      return { subject, avgPct: Math.round(avgPct), bestGrade, exams: data.marks.length };
    });
  };
  ...
  const getOverallAvg = (studentId: string) => {
    ...
    const avg = studentGrades.reduce((sum, g) => sum + (g.marks / g.maxMarks) * 100, 0) / studentGrades.length;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">My Children&apos;s Details</h2>
      </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start gap-6">
                    ...
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                          <Progress value={att.percentage} className="h-2 w-16 [&>div]:bg-amber-500" />
                          <p className={`text-sm font-bold ${att.percentage >= 80 ? 'text-emerald-600 dark:text-emerald-400' : att.percentage >= 60 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'}`}>
                            {att.percentage}%
                          </p>
                        </div>
                      </div>
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-bold text-gray-900 dark:text-gray-100">{overall.avg}%</span>
                          <Badge
                            variant="outline"
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{subjectPerf.length}</p>
                      </div>
                    </div>
                  </div>
                </CardContent>
                      ...
                      {subjectPerf.map((sp) => (
                        <div key={sp.subject} className="space-y-1.5">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="text-xs text-muted-foreground">{sp.exams} exams</span>
                              <Badge
                                variant="outline"
                          ...
                          <div className="flex items-center gap-3">
                            <Progress value={sp.avgPct} className="h-2.5 flex-1 [&>div]:bg-amber-500" />
                            <span className="text-xs font-semibold text-gray-600 dark:text-gray-400 w-10 text-right">{sp.avgPct}%</span>
                          </div>
                        </div>
                      ))}
                  ...
                  <CardContent className="p-4 pt-0">
                    <Table>
                      ...
                      <TableBody>
                        {recentGrades.map((g) => (
                          <TableRow key={g.id}>
                            <TableCell className="text-sm py-2">{g.subjectName}</TableCell>
                            <TableCell className="text-sm text-muted-foreground py-2 capitalize">{g.examType}</TableCell>
