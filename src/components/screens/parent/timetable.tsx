...
import {
  Clock, BookOpen, CalendarDays, User, MapPin,
} from 'lucide-react';
    ...
    return (
      <Card className="rounded-xl shadow-sm">
        <CardContent className="py-16 text-center">
          ...
          <p className="text-sm font-medium text-gray-500 dark:text-gray-400">No children found</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">No students are linked to your account.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">Class Timetable</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">View your children&apos;s weekly schedules</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card className="rounded-xl shadow-sm">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{selectedDaySlots.length} Periods</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">{DAY_FULL_LABELS[selectedDay]}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-xl shadow-sm">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">School hours</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {timeSlots.length === 0 ? (
        <Card className="rounded-xl shadow-sm">
          <CardContent className="py-16 text-center">
            ...
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">No timetable configured</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Contact the school administrator</p>
          </CardContent>
        </Card>
      ) : (
        <Card className="rounded-xl shadow-sm">
            ...
            <ScrollArea className="max-h-[500px]">
              <div className="space-y-3">
                {timeSlots.map((ts) => {
                  ...
                  if (!slot) {
                    return (
                      <div key={`${selectedDay}-${ts.start}`} className="flex items-center gap-4 p-3 rounded-lg border border-dashed border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/30">
                  ...
                  const colorClass = subjectColorMap[slot.subjectName] || 'bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700';

                  return (
                    <div key={slot.id} className="flex items-center gap-4 p-3 rounded-lg border border-gray-200 dark:border-gray-700 hover:shadow-sm transition-all">
