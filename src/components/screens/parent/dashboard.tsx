...
import {
  Users, Bell, DollarSign, Calendar, GraduationCap,
  TrendingUp, Clock, AlertTriangle, CheckCircle2, ArrowRight
} from 'lucide-react';

export function ParentDashboard() {
  const { currentUser } = useAppStore();
  const { data, isLoading, isError, error } = useParentDashboard(currentUser?.name || '');
        ...
        <Card className="rounded-xl shadow-sm">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{children.length}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="rounded-xl shadow-sm">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{parentNotices.length}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="rounded-xl shadow-sm">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                ${pendingFees.reduce((s, f) => s + f.amount - f.paidAmount, 0).toLocaleString()}
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Your Children</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {children.map((student) => {
              const attPct = getAttendanceForStudent(student.name);
              const { avg: avgPct, grade } = getAvgGradeForStudent(student.name);
              return (
                <Card key={student.id} className="rounded-xl shadow-sm hover:shadow-md transition-shadow">
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <Badge variant="outline" className="text-xs border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400">
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex items-center justify-between text-sm mb-1">
                          ...
                          <span className={`font-medium ${attPct >= 80 ? 'text-emerald-600' : attPct >= 60 ? 'text-amber-600' : 'text-red-600'}`}>
                            {attPct}%
                          </span>
                        </div>
                        <Progress value={attPct} className="h-2 [&>div]:bg-amber-500" />
                      </div>
                      <div>
                        <div className="flex items-center justify-between text-sm mb-1">
                          <span className="text-muted-foreground">Average Score</span>
                          <span className="font-medium text-gray-900 dark:text-gray-100">{avgPct}% ({grade})</span>
                        </div>
                        <Progress value={avgPct} className="h-2 [&>div]:bg-blue-500" />
                      </div>
                    </div>
                    <Separator className="my-3" />
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        {student.gender === 'male' ? '👦' : '👧'} {student.gender === 'male' ? 'Male' : 'Female'}
                      </span>
                      {student.dateOfBirth && (
                        <span>DOB: {new Date(student.dateOfBirth).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                      )}
                    </div>
                  </CardContent>
                </Card>
            ...
            <Card className="rounded-xl shadow-sm border-red-200 dark:border-red-800 bg-red-50/50 dark:bg-red-900/20">
              <CardHeader className="p-4 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-sm font-semibold text-red-800">Fee Payment Overdue</CardTitle>
                </div>
              </CardHeader>
              <CardContent className="p-4 pt-0 space-y-2">
                {overdueFees.slice(0, 3).map((fee) => (
                  <div key={fee.id} className="flex items-center justify-between text-sm">
                    <div>
                      <p className="font-medium text-red-900">{fee.studentName}</p>
                      <p className="text-xs text-red-600">{fee.type} - Due: {fee.dueDate}</p>
                    </div>
                    <Badge variant="destructive" className="text-xs">${fee.amount}</Badge>
                  </div>
                ))}
              ...
              {children.map((student) => {
                ...
                return (
                  <div key={student.id} className="flex items-center justify-between">
                    <span className="text-sm text-gray-600 dark:text-gray-400 truncate">{student.name}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">{avg}%</span>
                      <Badge
                        variant="outline"
