...
export function ParentFees() {
    ...
    async function fetchData() {
      try {
        ...
        if (parentStudents.length > 0) {
          ...
          const feesRes = await fetch(`/api/fees?studentId=${parentStudents[0].id}`);
  ...
  useEffect(() => {
    ...
    async function fetchAllFees() {
      try {
        const promises = allStudentIds.map(id => fetch(`/api/fees?studentId=${id}`));
  ...
  const summary = useMemo(() => {
    ...
    const paid = allChildrenFees.filter(f => f.status === 'paid').reduce((sum, f) => sum + f.paidAmount, 0);
    const pending = allChildrenFees.filter(f => f.status === 'pending').reduce((sum, f) => sum + (f.amount - f.paidAmount), 0);
    const overdue = allChildrenFees.filter(f => f.status === 'overdue').reduce((sum, f) => sum + (f.amount - f.paidAmount), 0);
  ...
  const handlePayNow = (feeId: string) => {
    ...
    setAllChildrenFees(prev => prev.map(f => {
      if (f.id === feeId) {
        return { ...f, status: 'paid', paidAmount: f.amount };
      }
      return f;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Fee Management</h2>
      </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-amber-600 dark:text-amber-400">${summary.pending.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {students.map((student) => {
          ...
          return (
            <TabsContent key={student.id} value={student.id} className="mt-6">
              <Card className="rounded-xl shadow-sm">
                <CardHeader>
                  <CardTitle className="text-base">Fee Details — {student.name}</CardTitle>
                  <CardDescription>{currentFees.length} fee records</CardDescription>
                </CardHeader>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="hidden md:table-cell">Paid Amount</TableHead>
