...
import {
  Crown, Check, Star, Zap, Shield, CreditCard, Calendar,
  ArrowRight, Gift, Clock, Users, BookOpen, TrendingUp,
  CheckCircle2, CircleDollarSign, Receipt, Loader2, XCircle
} from 'lucide-react';
...
const ADDONS: Addon[] = [
  {
    ...
    features: ['Real-time GPS tracking', 'ETA notifications', 'Geofence alerts'],
  },
  {
    id: 'meals',
    name: 'Meal Plan',
    ...
    features: ['Daily lunch included', 'Nutritional reports', 'Special diet options'],
  },
];
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        const matched = parents.find((p: { email: string }) => p.email === currentUser.email);
        if (!matched) {
          setLoading(false);
          return;
        }
    ...
    <div className="space-y-8">
            ...
            >
              {plan.badge && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold text-white ${plan.badgeColor}`}>
                    {plan.badge}
                  </span>
                </div>
              )}
              ...
              <CardHeader className={`pt-8 ${isPopular ? 'pb-2' : ''}`}>
                <div className="flex items-center justify-between">
                  <div className={`h-12 w-12 rounded-xl flex items-center justify-center ${
                    isPopular ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400' :
                    isActive ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' :
      ...
      <div>
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Add extra features to any subscription</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
          {ADDONS.map((addon) => {
            ...
            return (
              <Card key={addon.id} className="rounded-xl border-gray-200 dark:border-gray-700 hover:border-amber-200 dark:hover:border-amber-800 hover:shadow-md transition-all">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-bold text-gray-900 dark:text-gray-100">₹{addon.price}</span>
                      </div>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{addon.description}</p>
                      <div className="flex flex-wrap gap-1.5 mt-3">
                        {addon.features.map((f) => (
                          <span key={f} className="inline-flex items-center text-[11px] text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-full px-2 py-0.5">
                            {f}
                          </span>
                        ))}
                      </div>
          ...
          <CardContent className="p-0">
            {allSubscriptions.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-gray-400 dark:text-gray-500">
                ...
                <p className="text-xs mt-1">Your past subscriptions will appear here</p>
              </div>
            ) : (
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50/80 dark:bg-gray-800/50">
                      <TableHead className="font-semibold text-gray-600 dark:text-gray-400">Plan</TableHead>
                      <TableHead className="font-semibold text-gray-600 dark:text-gray-400">Amount</TableHead>
                      <TableHead className="font-semibold text-gray-600 dark:text-gray-400">Date</TableHead>
                      <TableHead className="font-semibold text-gray-600 dark:text-gray-400">Status</TableHead>
                      <TableHead className="font-semibold text-gray-600 dark:text-gray-400">Transaction ID</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
            ...
            <div className="space-y-4 py-2">
              <Card className="border-gray-200 dark:border-gray-700">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-gray-900 dark:text-gray-100">{purchasingPlan.name} Plan</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">{purchasingPlan.period}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xl font-bold text-gray-900 dark:text-gray-100">
                        {purchasingPlan.price === 0 ? 'Free' : `₹${purchasingPlan.price}`}
                      </p>
                      {purchasingPlan.originalPrice && (
                        <p className="text-xs text-gray-400 dark:text-gray-500 line-through">₹{purchasingPlan.originalPrice}</p>
                      )}
                    </div>
                  </div>
                </CardContent>
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-xs font-medium text-gray-500 dark:text-gray-400 block mb-1.5">Card Number</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-medium text-gray-500 dark:text-gray-400 block mb-1.5">Expiry</label>
                    <div className="h-10 w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 px-3 flex items-center text-sm text-gray-400 dark:text-gray-500">
                      12/27
                    </div>
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-500 dark:text-gray-400 block mb-1.5">CVV</label>
                    <div className="h-10 w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 px-3 flex items-center text-sm text-gray-400 dark:text-gray-500">
                      •••
                    </div>
                  </div>
                ...
                <div>
                  <label className="text-xs font-medium text-gray-500 dark:text-gray-400 block mb-1.5">Cardholder Name</label>
                  <div className="h-10 w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 px-3 flex items-center text-sm text-gray-400 dark:text-gray-500">
