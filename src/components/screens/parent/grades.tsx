...
import {
  TrendingUp, Award, BookOpen, BarChart3, Target, Star
} from 'lucide-react';
...
export function ParentGrades() {
  ...
  const getOverallStats = (studentId: string) => {
    const studentGrades = getGradesForStudent(studentId);
    if (studentGrades.length === 0) return { avg: 0, grade: 'N/A', highest: { subject: 'N/A', pct: 0 }, lowest: { subject: 'N/A', pct: 0 }, totalExams: 0 };
    ...
    return { avg: Math.round(avg), grade, highest, lowest, totalExams: studentGrades.length };
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Children&apos;s Grades</h2>
      </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <Card className="rounded-xl shadow-sm">
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{stats.avg}%</p>
                ...
                <Card className="rounded-xl shadow-sm">
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{stats.grade}</p>
                ...
                <Card className="rounded-xl shadow-sm">
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="text-lg font-bold text-gray-900 dark:text-gray-100 truncate">{stats.highest.subject}</p>
                    <p className="text-xs text-muted-foreground">Best Subject ({Math.round(stats.highest.pct)}%)</p>
                  </CardContent>
                </Card>
                <Card className="rounded-xl shadow-sm">
                  <CardContent className="p-4 text-center">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <Card className="lg:col-span-3 rounded-xl shadow-sm">
                  <CardHeader className="p-4 pb-2">
                    ...
                    <CardDescription>All exam results for {student.name}</CardDescription>
                  </CardHeader>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {studentGrades.map((g, i) => {
                            ...
                            return (
                              <TableRow key={`${g.id}-${i}`}>
                                <TableCell className="text-sm py-2 font-medium">{g.subjectName}</TableCell>
                                <TableCell className="text-sm py-2 text-muted-foreground capitalize">
                                  <Badge variant="outline" className="text-[10px] mr-1">{g.examType}</Badge>
                                </TableCell>
                                <TableCell className="text-sm py-2 text-right font-medium">
                                  {g.marks}<span className="text-muted-foreground">/{g.maxMarks}</span>
                                </TableCell>
