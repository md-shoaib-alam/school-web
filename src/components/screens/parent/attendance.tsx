...
function getMonthlyData(records: AttendanceRecord[]) {
  const now = new Date();
  const months: { month: string; present: number; absent: number; late: number }[] = [];
...
function getCalendarData(records: AttendanceRecord[]) {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    days.push({
      ...
      status: (dayOfWeek === 0 || dayOfWeek === 6) ? null : (statusMap.get(dateStr) || null),
    });
  }
  return days;
}
...
function getCalendarCellColor(status: string | null) {
  if (!status) return 'bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500';
  switch (status) {
...
export function ParentAttendance() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setAllAttendance(attendanceData.filter((a: AttendanceRecord) => studentIds.includes(a.studentId)));
  ...
  const stats = useMemo(() => getAttendanceStats(studentAttendance), [studentAttendance]);
  const monthlyData = useMemo(() => getMonthlyData(studentAttendance), [studentAttendance]);
  const calendarDays = useMemo(() => getCalendarData(studentAttendance), [studentAttendance]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        {students.map((student) => {
          const currentAttendance = allAttendance.filter(a => a.studentId === student.id);
            ...
            <TabsContent key={student.id} value={student.id} className="space-y-6 mt-6">
                  ...
                  <CardContent className="p-6 flex flex-col items-center justify-center">
                    <div className="relative w-32 h-32">
                      <svg className="w-32 h-32 -rotate-90" viewBox="0 0 128 128">
                        <circle cx="64" cy="64" r="56" fill="none" stroke="#f3f4f6" strokeWidth="10" className="dark:stroke-gray-700" />
                        <circle
                          cx="64" cy="64" r="56" fill="none"
                          stroke={currentStats.percentage >= 80 ? '#10b981' : currentStats.percentage >= 60 ? '#f59e0b' : '#ef4444'}
                          strokeWidth="10"
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className={`text-3xl font-bold ${currentStats.percentage >= 80 ? 'text-emerald-600 dark:text-emerald-400' : currentStats.percentage >= 60 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'}`}>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{currentStats.present}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-2xl font-bold text-red-600 dark:text-red-400">{currentStats.absent}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-2xl font-bold text-amber-600 dark:text-amber-400">{currentStats.late}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              <Card className="rounded-xl shadow-sm">
                <CardHeader className="p-4 pb-0">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-sm font-semibold">Monthly Attendance (Last 6 Months)</CardTitle>
                  </div>
                  <CardDescription className="text-xs">Present, Absent, and Late breakdown by month</CardDescription>
                </CardHeader>
                <CardContent className="p-4">
                  <div className="h-64 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={currentMonthly} barGap={2} barCategoryGap="20%">
              ...
              <Card className="rounded-xl shadow-sm">
                <CardHeader className="p-4 pb-2">
                  <div className="flex items-center gap-2">
                    <Calendar className="h-5 w-5 text-amber-600" />
                    <CardTitle className="text-sm font-semibold">30-Day Attendance Calendar</CardTitle>
                  </div>
                  <CardDescription className="text-xs">Last 30 days attendance overview</CardDescription>
                </CardHeader>
                <CardContent className="p-4">
                  {/* Legend */}
                  <div className="flex flex-wrap gap-3 mb-4">
                    {[
                      ...
                      { label: 'Weekend / No Data', color: 'bg-gray-200 dark:bg-gray-700' },
                    ].map(item => (
                  ...
                  <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 gap-2">
                    {currentCalendar.map((day, i) => (
                      <div
                        ...
                        title={day.status ? day.status.charAt(0).toUpperCase() + day.status.slice(1) : (day.day === 'S' || day.day === 'S' ? 'Weekend' : 'No data')}
                      >
