...
function getActionCategory(action: string): string {
  ...
  if (action.includes('SUSPEND') || action.includes('DISABLE')) return 'SUSPEND';
  return 'VIEW';
}
...
export function SuperAdminAuditLogs() {
  ...
  // Fetch via GraphQL
  const { data, isLoading: loading, error: fetchError } = useAuditLogs({
          ...
          <>
            <Card className="hover:shadow-md transition-shadow">
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{totalLogs}</p>
              </CardContent>
            </Card>
            {data?.actionTypes.slice(0, 4).map((at) => {
              const category = getActionCategory(at.action);
              return (
                <Card key={at.action} className="hover:shadow-md transition-shadow">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2 mb-1">
                      <Activity className="h-4 w-4 text-muted-foreground" />
                      <p className="text-xs text-muted-foreground font-medium truncate">{at.action.replace(/_/g, ' ')}</p>
                    </div>
                    <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{at.count}</p>
                  </CardContent>
                </Card>
              );
            })}
        ...
        <CardContent>
          ...
          {loading ? (
            ...
            <div className="text-center py-10">
              <p className="text-red-500 font-medium">Error loading audit logs</p>
            ...
            <>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredLogs.map((log) => {
                        ...
                        return (
                          <TableRow key={log.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900/50 transition-colors">
                            ...
                            <TableCell>
                              <span className="text-sm font-medium text-gray-700 dark:text-gray-300 capitalize">{log.resource}</span>
                            </TableCell>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-4">
                <p className="text-sm text-muted-foreground">
                  ...
                  {' '}of{' '}
                  <span className="font-medium text-gray-900 dark:text-gray-100">{totalLogs}</span>
                  {' '}entries
                </p>
                <div className="flex items-center gap-1">
                  <Button
