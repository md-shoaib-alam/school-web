...
import {
  Building2, Plus, Search, Edit, Trash2, Eye, MoreVertical, Filter,
  Users, GraduationCap, UserCheck, CreditCard, Crown,
  LayoutGrid, List, Globe, Phone, Mail, MapPin, Calendar,
  Activity, Ban, CheckCircle2, XCircle, ChevronLeft, ChevronRight, Database, ShieldCheck, Loader2, Lock, EyeOff,
} from 'lucide-react';
import { SchoolDetail } from './school-detail';
import { format } from 'date-fns';
import { useTenants, useCreateTenant, useUpdateTenant, useDeleteTenant, useToggleTenantStatus, useCreateUser } from '@/lib/graphql/hooks';
...
const StatCard = memo(function StatCard({ title, value, icon, iconBg, iconColor, trend }: { title: string; value: number; icon: React.ReactNode; iconBg: string; iconColor: string; trend: string | null }) {
  return (
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="flex items-baseline gap-2">
              ...
              {trend && <span className="text-[10px] text-emerald-600 font-medium">+{trend}</span>}
            </div>
          </div>
          <div className={`h-10 w-10 sm:h-12 sm:w-12 rounded-xl ${iconBg} ${iconColor} flex items-center justify-center shrink-0`}>
            {icon}
          </div>
        </div>
      </CardContent>
...
const PlanBadge = memo(({ plan }: { plan: string }) => {
  ...
  return (
    <Badge variant="outline" className={`${config.bg} ${config.text} ${config.border} border text-[10px] uppercase tracking-wider py-0.5 px-2 font-semibold`}>
...
const StatusBadge = memo(({ status }: { status: string }) => {
  ...
  return (
    <Badge variant="outline" className={`${config.bg} ${config.text} gap-1 text-[10px] py-0.5 px-2 border-none`}>
      {config.icon}
      {status.toUpperCase()}
    </Badge>
  );
...
const InfoItem = memo(({ icon, label, value }: { icon: React.ReactNode; label: string; value: string | null | undefined }) => {
  return (
    <div className="flex items-start gap-2 p-2 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
      <div className="text-muted-foreground mt-0.5">{icon}</div>
      <div className="min-w-0">
        <p className="text-[10px] text-muted-foreground uppercase tracking-tight">{label}</p>
...
const UsageStat = memo(({ icon, label, current, max, color, isCurrency = false }: { icon: React.ReactNode; label: string; current: number; max: number | null; color: string; isCurrency?: boolean }) => {
...
const TenantCard = memo(function TenantCard({
  tenant, onView, onEdit, onToggleStatus, onDelete, onManageData, onAddAdmin,
}: {
  tenant: Tenant;
  onView: () => void;
  onEdit: () => void;
      ...
      <CardContent className="p-5 space-y-4">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 hover:bg-rose-50 dark:hover:bg-rose-950/50">
                <MoreVertical className="h-4 w-4" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuItem onClick={onView}><Eye className="h-4 w-4 mr-2" />Details</DropdownMenuItem>
              <DropdownMenuItem onClick={onManageData}><Database className="h-4 w-4 mr-2" />Manage Data</DropdownMenuItem>
              <DropdownMenuItem onClick={() => onAddAdmin(tenant)}><ShieldCheck className="h-4 w-4 mr-2 text-indigo-600" />Create Admin</DropdownMenuItem>
              <DropdownMenuItem onClick={onEdit}><Edit className="h-4 w-4 mr-2" />Edit</DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={onToggleStatus}>
                {tenant.status === 'active' ? <><Ban className="h-4 w-4 mr-2" />Suspend</> : <><CheckCircle2 className="h-4 w-4 mr-2" />Activate</>}
              </DropdownMenuItem>
              <DropdownMenuSeparator />
        ...
        <div className="grid grid-cols-3 gap-2 py-1">
          <div className="text-center py-2 px-1 rounded-xl bg-muted/40 border border-muted-foreground/5">
            ...
            <p className="text-sm font-bold leading-tight">{tenant.studentCount}</p>
            <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-tight">Students</p>
          </div>
          <div className="text-center py-2 px-1 rounded-xl bg-muted/40 border border-muted-foreground/5">
            <Users className="h-4 w-4 mx-auto text-blue-500 mb-1" />
            <p className="text-sm font-bold leading-tight">{tenant.teacherCount}</p>
            <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-tight">Teachers</p>
          </div>
          <div className="text-center py-2 px-1 rounded-xl bg-muted/40 border border-muted-foreground/5">
            <UserCheck className="h-4 w-4 mx-auto text-emerald-500 mb-1" />
            <p className="text-sm font-bold leading-tight">{tenant.parentCount}</p>
            <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-tight">Parents</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 pt-1">
          <div className="flex gap-2">
            <Button variant="outline" size="sm" className="flex-1 h-9 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/30" onClick={onView}>View Details</Button>
            <Button variant="outline" size="sm" className="flex-1 h-9 rounded-lg hover:bg-emerald-50 dark:hover:bg-emerald-950/30" onClick={onManageData}>Live Data</Button>
          </div>
          <Button 
...
export function SuperAdminTenants() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingTenant) {
        await updateTenant.mutateAsync({ id: editingTenant.id, data: formData as unknown as Record<string, unknown> });
      } else {
  ...
  const handleCreateAdmin = async () => {
    if (!targetTenantForAdmin || !adminFormData.name || !adminFormData.email || !adminFormData.password) {
      return;
    }
    setSubmitting(true);
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Button className="bg-rose-600 hover:bg-rose-700 text-white" onClick={handleOpenAddDialog}>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-16 text-center">
            ...
            {!search && planFilter === 'all' && statusFilter === 'all' && (
              <Button className="mt-4 bg-rose-600 hover:bg-rose-700 text-white" onClick={handleOpenAddDialog}>
                <Plus className="h-4 w-4 mr-2" />
                Add School
              </Button>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {paginatedTenants.map((tenant) => (
            <TenantCard
              key={tenant.id}
              tenant={tenant}
              onView={() => { setViewingTenant(tenant); setDetailDialogOpen(true); }}
              onEdit={() => handleOpenEditDialog(tenant)}
              onToggleStatus={() => handleToggleStatus(tenant)}
              onDelete={() => { setDeletingTenant(tenant); setDeleteDialogOpen(true); }}
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="hidden lg:table-cell">Parents</TableHead>
                    <TableHead className="hidden lg:table-cell">Revenue</TableHead>
                    <TableHead className="hidden xl:table-cell">Subscriptions</TableHead>
                ...
                <TableBody>
                  {paginatedTenants.map((tenant) => (
                    <TableRow key={tenant.id} className="hover:bg-rose-50 dark:bg-rose-900/30/30 transition-colors">
                      <TableCell>
                        <div className="flex items-center gap-3">
                      ...
                      <TableCell className="hidden md:table-cell">
                        <span className="text-sm font-medium">{tenant.studentCount}</span>
                        <span className="text-xs text-muted-foreground">/{tenant.maxStudents}</span>
                      </TableCell>
                      <TableCell className="hidden md:table-cell">
                        <span className="text-sm font-medium">{tenant.teacherCount}</span>
                        <span className="text-xs text-muted-foreground">/{tenant.maxTeachers}</span>
                      </TableCell>
                      <TableCell className="hidden lg:table-cell">
                        <span className="text-sm font-medium">{tenant.parentCount}</span>
                        <span className="text-xs text-muted-foreground">/{tenant.maxParents}</span>
                      </TableCell>
                      <TableCell className="hidden lg:table-cell">
                        <span className="text-sm font-semibold">${tenant.totalRevenue.toLocaleString()}</span>
                      </TableCell>
                      <TableCell className="hidden xl:table-cell">
                        <span className="text-sm">{tenant.activeSubscriptions}</span>
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48">
                            <DropdownMenuItem onClick={() => { setViewingTenant(tenant); setDetailDialogOpen(true); }}>
                            ...
                            <DropdownMenuItem onClick={() => handleOpenAddAdmin(tenant)}>
      ...
      <Dialog open={formDialogOpen} onOpenChange={setFormDialogOpen}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingTenant ? 'Edit School' : 'Add New School'}</DialogTitle>
            <DialogDescription>
              {editingTenant
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-4 p-3 rounded-lg bg-muted/50">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold capitalize">{viewingTenant.plan}</span>
                    </div>
                    <Separator orientation="vertical" className="h-6" />
                    <div className="text-sm text-muted-foreground">
                      <span className="font-medium text-foreground">{viewingTenant._count.users}</span> total users
                    </div>
                    <Separator orientation="vertical" className="h-6" />
                    <div className="text-sm text-muted-foreground">
                      <span className="font-medium text-foreground">{viewingTenant._count.notices}</span> notices
                    </div>
                    <Separator orientation="vertical" className="h-6" />
                    <div className="text-sm text-muted-foreground">
                      <span className="font-medium text-foreground">{viewingTenant._count.events}</span> events
                    </div>
                  </div>
                </div>
              </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete <strong>{deletingTenant?.name}</strong>? This will permanently
              remove the school and all associated data including users, classes, notices, and subscriptions.
              This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => { setDeleteDialogOpen(false); setDeletingTenant(null); }}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
      ...
      <Dialog open={adminModalOpen} onOpenChange={setAdminModalOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Set up the administrative account for <span className="text-foreground font-semibold uppercase text-xs px-1.5 py-0.5 rounded bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/50">{targetTenantForAdmin?.name}</span>
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="admin-name" className="text-xs font-bold uppercase tracking-wider text-muted-foreground pl-1">Full Name</Label>
              <Input
                id="admin-name"
            ...
            <div className="grid gap-2">
              <Label htmlFor="admin-email" className="text-xs font-bold uppercase tracking-wider text-muted-foreground pl-1">Email Address</Label>
            ...
            <div className="grid gap-2">
              <Label htmlFor="admin-phone" className="text-xs font-bold uppercase tracking-wider text-muted-foreground pl-1">Phone Number (Optional)</Label>
              <div className="relative group">
            ...
            <div className="grid gap-2">
              <Label htmlFor="admin-password" className="text-xs font-bold uppercase tracking-wider text-muted-foreground pl-1">Initial Password</Label>
              <div className="relative group">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant="ghost" onClick={() => setAdminModalOpen(false)} disabled={submitting} className="rounded-xl">
              Cancel
            </Button>
            <Button 
