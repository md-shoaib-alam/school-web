      ...
      <Card className="border shadow-sm bg-card">
        ...
        <CardContent className="space-y-4">
          {loading ? (
            ...
            <>
              <div className="flex items-center justify-between p-3.5 rounded-2xl bg-emerald-50/50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 transition-colors">
                ...
                <span className="text-2xl font-semibold text-emerald-700 dark:text-emerald-400">{data?.tenants.active ?? 0}</span>
              </div>

              <div className="flex items-center justify-between p-3.5 rounded-2xl bg-purple-50/50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800/50 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="size-10 rounded-xl bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center">
                    <Clock className="size-5 text-purple-600" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-purple-900 dark:text-purple-100">Trial</p>
                    <p className="text-xs text-purple-600 font-medium">Evaluation</p>
                  </div>
                </div>
                <span className="text-2xl font-semibold text-purple-700 dark:text-purple-400">{data?.tenants.trial ?? 0}</span>
              </div>

              <div className="flex items-center justify-between p-3.5 rounded-2xl bg-red-50/50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/50 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="size-10 rounded-xl bg-red-100 dark:bg-red-900/30 flex items-center justify-center">
                    <AlertTriangle className="size-5 text-red-600" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-red-900 dark:text-red-100">Suspended</p>
                    <p className="text-xs text-red-600 font-medium">Attention Required</p>
                  </div>
                </div>
                <span className="text-2xl font-semibold text-red-700 dark:text-red-400">{data?.tenants.suspended ?? 0}</span>
              </div>
            </>
          )}
        </CardContent>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {userDistributionData.map((item) => (
                  <div key={item.name} className="p-2.5 rounded-xl bg-muted/30 border border-border text-center">
                    <p className="text-lg font-semibold text-foreground leading-none mb-1">{item.value.toLocaleString()}</p>
...
function ActionButton({ icon, label, sub, color, onClick }: any) {
  return (
    <Button
      variant="outline"
      className="w-full justify-start gap-4 h-[60px] rounded-2xl border-2 border-border hover:border-emerald-500/30 hover:bg-emerald-50/10 transition-all group"
      onClick={onClick}
    >
      <div className={`size-10 rounded-xl ${color} flex items-center justify-center text-white shadow-sm`}>
        {icon}
      </div>
