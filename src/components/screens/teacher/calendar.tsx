...
const ROLE_FILTER = ["all", "teacher"] as const;
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/40 dark:bg-gray-900/40 p-1.5 rounded-2xl border border-white/20 dark:border-gray-800/20 backdrop-blur-md shadow-sm">
        <div className="flex items-center gap-3 px-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 uppercase tracking-tight">Staff Calendar</h2>
            <p className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest">Faculty Portal</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <CardHeader className="p-5 border-b border-gray-50/50 dark:border-gray-800/50 flex flex-row items-center justify-between bg-gray-50/30 dark:bg-gray-950/20">
            ...
            <div className="flex flex-wrap gap-1.5">
              <Button
                ...
                className={`h-7 text-[10px] font-black px-3 rounded-lg shadow-none ${activeTypeFilter === "all" ? "bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-500/20" : "text-gray-400"}`}
              ...
              {availableTypes.map((type) => (
                <Button
                  ...
                  size="sm"
                  variant="ghost"
                  className={`h-7 text-[10px] font-black px-3 rounded-lg shadow-none ${activeTypeFilter === type ? "" : "text-gray-400 hover:text-blue-500"}`}
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        hover:z-20 hover:bg-blue-50/30 dark:hover:bg-blue-900/10 focus:outline-none
                        ${isToday ? "bg-blue-50/20 dark:bg-blue-900/10 shadow-inner" : ""}
                        ${isSelected ? "ring-2 ring-blue-500 ring-inset bg-blue-50/50 dark:bg-blue-900/20 shadow-lg" : ""}
                      `}
                    >
                      <span className={`
                        text-xs sm:text-sm font-black flex items-center justify-center transition-all
                        ${isToday ? "bg-blue-600 text-white w-7 h-7 sm:w-8 sm:h-8 rounded-lg shadow-xl shadow-blue-500/30" : "text-gray-900 dark:text-gray-100"}
                        ${isSelected && !isToday ? "text-blue-600 scale-110" : ""}
                      `}>
                        {day}
                      </span>
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <CardContent className="p-0">
              {!selectedDate || selectedEvents.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-24 px-10 text-center animate-in fade-in zoom-in-95">
                  <div className="w-16 h-16 rounded-[2rem] bg-gray-50 dark:bg-gray-800 border-2 border-dashed border-gray-200 dark:border-gray-700 flex items-center justify-center mb-6 transition-all hover:border-blue-500 hover:rotate-12 group">
                    <CalendarDays className="h-8 w-8 text-gray-300 dark:text-gray-600 transition-colors group-hover:text-blue-500" />
                  </div>
                  <h3 className="text-sm font-black text-gray-900 dark:text-gray-100 uppercase tracking-widest">
                    {selectedDate ? "Day is Clear" : "Review Required"}
                  </h3>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-3 max-w-[200px] leading-relaxed">
                    {selectedDate 
                      ? "No professional activities scheduled for this date. Focus on prep work or relax!" 
                      : "Select a date on the calendar to view your academic schedule."}
                  </p>
                </div>
              ) : (
                <div className="p-4 space-y-5">
                  <div className="px-2 flex items-center justify-between">
                    ...
                    <Badge variant="secondary" className="rounded-lg font-black text-[10px] bg-blue-50 text-blue-600 border-none px-2.5 py-1">
                  ...
                  <ScrollArea className="max-h-[520px] pr-3 -mr-2">
                    <div className="space-y-4">
                      {selectedEvents.map((event) => (
                        <div
                          key={event.id}
                          className="group relative bg-white dark:bg-gray-950 p-4 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-blue-500/40 hover:shadow-2xl hover:shadow-blue-500/5 transition-all duration-500"
                        >
                          <div
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              <h5 className="text-sm font-black text-gray-900 dark:text-gray-100 leading-snug group-hover:text-blue-600 transition-colors">
                                {event.title}
                              </h5>
                          ...
                          {event.location && (
                            <div className="mt-4 pt-3.5 border-t border-gray-50 dark:border-gray-800 flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                              <MapPin className="h-3.5 w-3.5 text-blue-500 transition-transform group-hover:scale-125" />
                              {event.location}
                            </div>
                          )}
                        </div>
          ...
          {/* Key Legend Card */}
          <Card className="rounded-2xl border-gray-100/50 dark:border-gray-800/50 bg-blue-500/5 p-5 backdrop-blur-md border border-blue-500/10">
            ...
            <div className="grid grid-cols-2 gap-3.5">
              {availableTypes.length > 0 ? (
                ...
                <p className="text-[10px] text-gray-400 italic font-bold">No active categories found.</p>
              )}
            </div>
          </Card>
        </div>
