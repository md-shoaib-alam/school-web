...
function formatTimeRange(date: string, endDate?: string, allDay?: boolean): string {
  if (allDay) return 'All Day';
  const d = new Date(date);
  const start = d.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' });
  if (!endDate) return start;
  const ed = new Date(endDate);
  const end = ed.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' });
  return `${start} – ${end}`;
}
...
export function TeacherCalendar() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/events?tenantId=${encodeURIComponent(currentTenantId)}&month=${monthStr}`)
      .then((res) => res.json())
      .then((json) => {
        const data: CalendarEvent[] = Array.isArray(json) ? json : (json.data ?? []);
        const filtered = data.filter((e) => ROLE_FILTER_SET.has(e.targetRole as string));
        setEvents(filtered);
      })
      .catch(() => setEvents([]))
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">School Calendar</h2>
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          View upcoming events, exams, holidays, and activities.
        </p>
      </div>
          ...
          <CardContent className="p-4 sm:p-6">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-1">
                <Button variant="ghost" size="icon" onClick={goToPrevMonth} className="h-9 w-9">
                ...
                <Button
                  ...
                  style={
                    activeTypeFilter === type
                      ? { backgroundColor: EVENT_TYPE_COLORS[type] || '#6b7280', color: '#fff', borderColor: 'transparent' }
                      : {}
                  }
                >
                  <span
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        ${isToday ? 'ring-2 ring-blue-500 ring-offset-1 dark:ring-offset-gray-900 bg-blue-50 dark:bg-blue-900/30' : ''}
                        ${isSelected && !isToday ? 'bg-blue-100/70 dark:bg-blue-900/50' : ''}
                        ${!isToday && !isSelected ? 'text-gray-700 dark:text-gray-300' : ''}
                        ${isToday ? 'text-blue-700 dark:text-blue-400 font-bold' : ''}
                      `}
                    >
                      <span className="text-xs sm:text-sm leading-none">{day}</span>
                      {dayEvents.length > 0 && (
                        <div className="flex gap-0.5 mt-0.5">
                          {dayEvents.slice(0, 3).map((ev) => (
                            <span
                              ...
                              style={{ backgroundColor: ev.color || EVENT_TYPE_COLORS[ev.type] || '#6b7280' }}
                            />
                          ))}
                          {dayEvents.length > 3 && (
                            <span className="text-[8px] text-gray-400 dark:text-gray-500 leading-none">+{dayEvents.length - 3}</span>
                          )}
                        </div>
                      )}
                    </button>
          ...
          <CardContent className="p-4 pt-0">
            {isLoading ? (
                ...
                {selectedEvents.map((event) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 mb-1.5">
                      ...
                      <span>{formatTimeRange(event.date, event.endDate, event.allDay)}</span>
                    </div>
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                {selectedDate ? (
                  <>
                    <p className="text-sm text-gray-400 dark:text-gray-500 font-medium">No events</p>
                  ...
                  <>
                    <p className="text-sm text-gray-400 dark:text-gray-500 font-medium">Pick a date</p>
                    <p className="text-xs text-gray-300 dark:text-gray-600 mt-1">
