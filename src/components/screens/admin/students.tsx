...
import {
  GraduationCap, Plus, Search, Trash2, Pencil, ChevronLeft, ChevronRight, Phone, Upload, Download, FileSpreadsheet, AlertCircle, CheckCircle2, Loader2, Eye,
} from 'lucide-react';
...
function PaginationPages({ currentPage, totalPages, onPageChange }: { currentPage: number; totalPages: number; onPageChange: (page: number) => void }) {
  ...
  return (
    <div className="flex items-center gap-1">
      {getPageNumbers().map((page, idx) => {
        if (page === 'ellipsis') {
          return (
            <span key={`ellipsis-${idx}`} className="px-1.5 text-muted-foreground text-sm select-none">
              ...
            </span>
          );
        }
...
export function AdminStudents() {
  ...
  const refetchStudents = () => queryClient.invalidateQueries({ queryKey: ['students'] });

  // Add student dialog
  const [addDialogOpen, setAddDialogOpen] = useState(false);
  const [formData, setFormData] = useState({
    name: '', email: '', phone: '', rollNumber: '',
    classId: '', gender: 'male', dateOfBirth: '',
  });
  ...
  const [importResult, setImportResult] = useState<{ success: boolean; imported: number; errors: number; total: number } | null>(null);
  ...
  const handleAddStudent = async () => {
    ...
    try {
      ...
      setFormData({ name: '', email: '', phone: '', rollNumber: '', classId: '', gender: 'male', dateOfBirth: '' });
      refetchStudents();
    } catch {
  ...
  const downloadSampleCSV = () => {
    const headers = 'name,email,phone,class (e.g. 10-A),roll_number,gender,date_of_birth';
    const sampleRows = [
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        ...
        <div className="flex gap-2 shrink-0">
          {canCreate && (
          <Button variant="outline" className="text-emerald-700 dark:text-emerald-400 border-emerald-300 dark:border-emerald-700 hover:bg-emerald-50 dark:hover:bg-emerald-900/30" onClick={() => { setImportDialogOpen(true); setImportFile(null); setImportResult(null); }}>
              ...
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {paginated.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={(canEdit || canDelete) ? 7 : 6} className="text-center py-12 text-muted-foreground">
                      ...
                      paginated.map((student) => (
                        <TableRow key={student.id} className="hover:bg-emerald-50/50 dark:hover:bg-emerald-900/20 transition-colors">
                          ...
                          <TableCell>
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-xs text-muted-foreground truncate md:hidden">{student.className}</p>
                              </div>
                            </div>
                          </TableCell>
                          <TableCell className="hidden md:table-cell">
                            <Badge variant="secondary" className="font-normal">{student.className}</Badge>
                          </TableCell>
                          <TableCell className="hidden sm:table-cell capitalize">{student.gender}</TableCell>
                          <TableCell className="hidden lg:table-cell text-sm">{student.parentName || '—'}</TableCell>
                          <TableCell className="hidden lg:table-cell text-sm">{student.phone || '—'}</TableCell>
                          {(canEdit || canDelete) && (
                            ...
                            <div className="flex items-center justify-end gap-1">
                              ...
                              <AlertDialog>
                                <AlertDialogTrigger asChild>
                                  <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-red-600" onClick={() => setDeletingId(student.id)}>
              ...
              {filtered.length > 0 && (
                <div className="flex flex-col sm:flex-row items-center justify-between px-4 py-3 border-t gap-3">
                  <p className="text-sm text-muted-foreground">
                    Showing <span className="font-medium">{Math.min((currentPage - 1) * ITEMS_PER_PAGE + 1, filtered.length)}</span>
                    {' to '}
      ...
      <Dialog open={importDialogOpen} onOpenChange={(open) => { setImportDialogOpen(open); if (!open) { setImportFile(null); setImportResult(null); } }}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Upload a CSV file to import students in bulk</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            >
              ...
              {importFile ? (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-xs text-muted-foreground">{(importFile.size / 1024).toFixed(1)} KB — Click or drop to replace</p>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-2">
            ...
            <div className="rounded-lg bg-muted/50 p-4 space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {['name', 'email', 'phone', 'class (e.g. 10-A)', 'roll_number', 'gender', 'date_of_birth'].map((col) => (
            ...
            {/* Import result */}
            {importResult && (
              <div className={`rounded-lg p-4 flex items-start gap-3 ${importResult.success ? 'bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800' : 'bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800'}`}>
                ...
                <div className="text-sm">
                  <p className="font-medium">{importResult.success ? 'Import completed' : 'Import completed with issues'}</p>
                  <p className="text-muted-foreground">
                    {importResult.imported} of {importResult.total} students imported successfully
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-2">
            <div className="grid gap-2">
              ...
              <Input id="edit-name" value={editFormData.name} onChange={(e) => setEditFormData({ ...editFormData, name: e.target.value })} placeholder="John Doe" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label htmlFor="edit-email">Email</Label>
                <Input id="edit-email" type="email" value={editFormData.email} onChange={(e) => setEditFormData({ ...editFormData, email: e.target.value })} placeholder="john@school.com" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="edit-rollNumber">Roll Number</Label>
                <Input id="edit-rollNumber" value={editFormData.rollNumber} onChange={(e) => setEditFormData({ ...editFormData, rollNumber: e.target.value })} placeholder="001" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input id="edit-phone" value={editFormData.phone} onChange={(e) => setEditFormData({ ...editFormData, phone: e.target.value })} placeholder="+1 234 567" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="edit-dob">Date of Birth</Label>
                <Input id="edit-dob" type="date" value={editFormData.dateOfBirth} onChange={(e) => setEditFormData({ ...editFormData, dateOfBirth: e.target.value })} />
              </div>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              disabled={editSubmitting || !editFormData.name || !editFormData.email || !editFormData.classId || !editFormData.rollNumber}
            >
              {editSubmitting ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input id="email" type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} placeholder="john@school.com" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="rollNumber">Roll Number</Label>
                <Input id="rollNumber" value={formData.rollNumber} onChange={(e) => setFormData({ ...formData, rollNumber: e.target.value })} placeholder="001" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input id="phone" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} placeholder="+1 234 567" />
              </div>
              <div className="grid gap-2">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={submitting || !formData.name || !formData.email || !formData.classId || !formData.rollNumber}
            >
