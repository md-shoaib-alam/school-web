...
import { BookOpen, Search, Plus, Trash2, Pencil, Loader2, Eye } from 'lucide-react';
import { toast } from 'sonner';
import { useModulePermissions } from '@/hooks/use-permissions';
import {
  useSubjects, useClassesMin, useTeachersMin,
  ...
  const subjectFormFields = (
    ...
    <div className="space-y-4 pt-2">
      <div>
        ...
        <Input value={value.name} onChange={e => onChange({ ...value, name: e.target.value })} placeholder="e.g. Mathematics" className="mt-1.5" />
      </div>
      <div>
        <Label>Subject Code *</Label>
        <Input value={value.code} onChange={e => onChange({ ...value, code: e.target.value })} placeholder="e.g. MATH-101" className="mt-1.5" />
      </div>
      <div>
        <Label>Class *</Label>
        <Select value={value.classId} onValueChange={v => onChange({ ...value, classId: v })}>
      ...
      <div>
        ...
        <Select value={value.teacherId} onValueChange={v => onChange({ ...value, teacherId: v })}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{subjects.length} subjects across all classes</p>
        </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-20 text-center">{(canEdit || canDelete) && 'Actions'}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((subject) => (
                      <TableRow key={subject.id} className="hover:bg-emerald-50/50 dark:hover:bg-emerald-900/20 transition-colors">
                        <TableCell>
                        ...
                        <TableCell>
                          <code className="px-2 py-0.5 rounded bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs font-mono">{subject.code}</code>
                        </TableCell>
                        <TableCell className="hidden sm:table-cell">
                          <Badge variant="outline" className="font-normal">{subject.className}</Badge>
                        </TableCell>
                        ...
                        <TableCell className="text-center">
                          {subject.teacherName && subject.teacherName !== 'Not Assigned' ? (
                            <Badge className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800 text-[10px]">Active</Badge>
                          ) : (
                            <Badge className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800 text-[10px]">Unassigned</Badge>
                          )}
                        </TableCell>
                        <TableCell>
                          {(canEdit || canDelete) && (
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          {subjectFormFields(editForm, (v) => setEditForm(prev => ({ ...prev, ...v })))}
          <DialogFooter>
            <Button variant="outline" onClick={() => { setEditOpen(false); setEditForm({ id: '', ...emptyForm }); }}>Cancel</Button>
            <Button onClick={handleEdit} disabled={updateMutation.isPending} className="bg-emerald-600 hover:bg-emerald-700 text-white">
