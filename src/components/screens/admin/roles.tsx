...
import { Plus, Pencil, Trash2, Shield, Users, Loader2, UserPlus, UserMinus, Search, X } from 'lucide-react';
    ...
    try {
      const [assignedRes, availableRes] = await Promise.all([
        ...
        fetch('/api/roles/users', {
          ...
          body: JSON.stringify({ tenantId: currentTenantId, excludeRoleId: role.id }),
        }),
      ]);
  ...
  const handleAssign = async (userId: string) => {
    ...
    try {
      const res = await fetch('/api/roles/users', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, roleId: assignRole.id, tenantId: currentTenantId }),
      });
      if (!res.ok) throw new Error();
      ...
      if (user) {
        ...
        setAssignedUsers(prev => [...prev, { ...user, customRoleId: assignRole.id }]);
      }
  ...
  const handleUnassign = async (userId: string) => {
    ...
    try {
      const res = await fetch('/api/roles/users', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, roleId: null, tenantId: currentTenantId }),
      });
      if (!res.ok) throw new Error();
    ...
    try {
      if (editingRole) {
        ...
        const res = await fetch('/api/roles', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ tenantId: currentTenantId, name, description, color, permissions }),
        });
        if (!res.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Create custom roles, assign them to staff, and control access</p>
        </div>
      ...
      {roles.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16 text-gray-400 dark:text-gray-500">
            ...
            <p className="text-sm mt-1">Create your first role to manage staff permissions</p>
            <Button onClick={openCreateDialog} variant="outline" className="mt-4 border-emerald-300 text-emerald-700 hover:bg-emerald-50 dark:hover:bg-emerald-900/20">
              <Plus className="h-4 w-4 mr-2" /> Create Role
            </Button>
          </CardContent>
        </Card>
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-emerald-50 hover:text-emerald-600 dark:hover:bg-emerald-900/30" onClick={() => openEditDialog(role)}>
                        <Pencil className="h-3.5 w-3.5" />
                      </Button>
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/30">
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Are you sure you want to delete &quot;{role.name}&quot;? {role._count.users > 0 ? `${role._count.users} staff member(s) will lose this role.` : 'This action cannot be undone.'}
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-5 py-2">
            <div className="space-y-2">
              ...
              <Input placeholder="e.g. Finance Manager" value={name} onChange={e => setName(e.target.value)} />
            </div>
            ...
            <div className="space-y-3">
                ...
                {PERMISSION_MODULES.map((mod, idx) => (
                  <Fragment key={mod.key}>
                    <div className={`flex items-center gap-2 px-3 py-2.5 text-sm ${idx % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50 dark:bg-gray-900/50'}`}>
                      ...
                      <span className="font-medium text-gray-700 dark:text-gray-300">{mod.label}</span>
                    </div>
                    {PERMISSION_ACTIONS.map(action => (
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" className="text-xs" onClick={() => {
                  ...
                  PERMISSION_MODULES.forEach(m => { all[m.key] = ['view', 'create', 'edit', 'delete']; });
                  setPermissions(all);
                }}>Select All</Button>
                <Button variant="outline" size="sm" className="text-xs" onClick={() => {
                  const viewOnly: Record<string, string[]> = {};
                  PERMISSION_MODULES.forEach(m => { viewOnly[m.key] = ['view']; });
                  setPermissions(viewOnly);
                }}>View Only</Button>
                <Button variant="outline" size="sm" className="text-xs" onClick={() => setPermissions({})}>Clear All</Button>
              </div>
            </div>
          </div>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button onClick={handleSave} disabled={saving || !name.trim()} className="bg-emerald-600 hover:bg-emerald-700 text-white">
                  ...
                  {assignedUsers.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {assignedUsers.map(user => (
                        <div key={user.id} className="flex items-center gap-3 p-3 rounded-lg border bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-200 dark:border-emerald-800/50">
                          <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ backgroundColor: assignRole?.color }}>
                          ...
                          >
                            {assigning === user.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <UserMinus className="h-3.5 w-3.5" />}
                          </Button>
                        </div>
                      ))}
                    </div>
                  ...
                  <ScrollArea className="max-h-[200px]">
                    {filteredAvailable.length === 0 ? (
                      <p className="text-sm text-gray-400 dark:text-gray-500 py-4 text-center">
                        {searchQuery ? 'No users found matching your search.' : 'All users already have a role assigned.'}
                      </p>
                    ) : (
                      <div className="space-y-1.5">
                        {filteredAvailable.map(user => (
                          <div key={user.id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm text-gray-800 dark:text-gray-200 truncate">{user.name}</p>
                              <p className="text-xs text-gray-400 dark:text-gray-500 truncate">{user.email}</p>
                            </div>
                            {user.customRole && (
                              <Badge variant="outline" className="text-[10px] shrink-0 hidden sm:inline-flex" style={{ borderColor: user.customRole.color + '40', color: user.customRole.color }}>
                            ...
                            <Button
                              ...
                              disabled={assigning === user.id}
                            >
                              {assigning === user.id ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1" /> : <UserPlus className="h-3 w-3 mr-1" />}
                              Assign
                            </Button>
                          </div>
                        ))}
