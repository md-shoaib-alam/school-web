...
export function AdminFees() {
  ...
  const totalPending = fees.filter((f) => f.status === 'pending').reduce((sum, f) => sum + (f.amount - f.paidAmount), 0);
  const totalOverdue = fees.filter((f) => f.status === 'overdue').reduce((sum, f) => sum + (f.amount - f.paidAmount), 0);
  ...
  const handleEdit = (fee: FeeRecord) => {
    ...
    setEditForm({
      ...
      paidDate: fee.status === 'paid' ? new Date().toISOString().split('T')[0] : '',
    });
    setEditOpen(true);
  };
    ...
    <div className="space-y-6">
          ...
          <Card key={card.label} className="hover:shadow-md transition-shadow">
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">${card.amount.toLocaleString()}</p>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        ...
        <Button
          ...
          onClick={() => { setAddForm({ ...emptyFeeForm }); setAddOpen(true); }}
        >
          <Plus className="h-4 w-4 mr-2" />
          Add Fee
        </Button>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-36 text-center">{(canEdit || canDelete) && 'Actions'}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filtered.length === 0 ? (
                          ...
                          <TableCell>
                            <div className="flex items-center gap-3">
                              <div className="h-8 w-8 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 flex items-center justify-center text-xs font-semibold">
                                {fee.studentName.split(' ').map((n) => n[0]).join('').slice(0, 2).toUpperCase()}
                              </div>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="grid gap-4 py-2">
            <div className="grid gap-2">
              <Label>Student *</Label>
              <Select value={addForm.studentId} onValueChange={(v) => setAddForm({ ...addForm, studentId: v })}>
                <SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={handleAdd}
              disabled={adding || !addForm.studentId || !addForm.amount || !addForm.dueDate}
            >
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Update fee record for {editingFee?.studentName}</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-2">
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
