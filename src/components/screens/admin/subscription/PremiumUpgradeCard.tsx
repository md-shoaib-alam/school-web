...
export interface PremiumUpgradeCardProps {
  title?: string;
  description?: React.ReactNode;
  subCaption?: React.ReactNode;
  imageSrc?: string;
  badgeText?: string;
  features?: UpgradeFeatureItem[];
...
const defaultFeatures: UpgradeFeatureItem[] = [
  {
    icon: <FileText className="size-4" />,
    iconBg: 'bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 border-blue-100 dark:border-blue-900/40',
    text: 'Professional A4 templates',
  },
  {
    icon: <Palette className="size-4" />,
    iconBg: 'bg-pink-50 dark:bg-pink-950/60 text-pink-600 dark:text-pink-400 border-pink-100 dark:border-pink-900/40',
  ...
  },
  {
    ...
    text: 'Library trackers & advanced tools',
  },
  {
    icon: <Printer className="size-4" />,
    iconBg: 'bg-violet-50 dark:bg-violet-950/60 text-violet-600 dark:text-violet-400 border-violet-100 dark:border-violet-900/40',
    text: 'High-quality PDF export & print',
  },
  {
    icon: <BarChart3 className="size-4" />,
    iconBg: 'bg-teal-50 dark:bg-teal-950/60 text-teal-600 dark:text-teal-400 border-teal-100 dark:border-teal-900/40',
    text: 'Bulk print and export options',
  },
];
...
export function PremiumUpgradeCard({
  ...
  primaryBtnText = 'Upgrade School Plan',
  secondaryBtnText = 'View Plan Details',
  onUpgrade,
  onViewPlan,
  className = '',
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-1/3 left-[36%] size-2.5 rounded-full bg-amber-400/80 blur-[0.5px] pointer-events-none hidden sm:block" />
      <div className="absolute top-1/2 left-8 size-2 rounded-full bg-purple-400/70 blur-[0.5px] pointer-events-none hidden sm:block" />
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center justify-center gap-6 sm:gap-8 lg:gap-12 w-full">
        ...
        <div className="w-full lg:w-[55%] flex flex-col items-center lg:items-start text-center lg:text-left justify-center">
          ...
          <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 mt-2 sm:mt-2.5 leading-relaxed max-w-lg">
            {description || (
              <>
                High-fidelity marksheet generation and printing is available in{' '}
          ...
          {/* 2-Column Feature Grid with individual tile cards */}
          <div className="grid grid-cols-2 gap-2 sm:gap-3 w-full mt-5 sm:mt-6">
            {features.map((feature, idx) => (
              <div
                key={idx}
                className="flex items-center gap-2 sm:gap-2.5 p-2 sm:p-2.5 rounded-2xl bg-zinc-50/90 dark:bg-zinc-900/60 border border-zinc-100/90 dark:border-zinc-800/80 hover:bg-zinc-100/70 dark:hover:bg-zinc-800/60 transition-colors"
              >
                <div
                  className={`size-8 sm:size-9 rounded-xl flex items-center justify-center shrink-0 border ${feature.iconBg}`}
                >
                  {feature.icon}
                </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-2.5 sm:gap-3 mt-6 sm:mt-8 w-full">
            <Button
              onClick={onUpgrade}
              className="w-full sm:w-auto flex-1 bg-blue-600 hover:bg-blue-700 active:scale-[0.98] text-white font-semibold h-12 px-6 rounded-2xl shadow-lg shadow-blue-500/25 transition-all flex items-center justify-center gap-2 cursor-pointer"
            >
              <Crown className="size-4.5 fill-white/20" />
