    ...
    <div className="print-container bg-white">
      <div className="relative cert-frame flex flex-col border-4 border-double border-amber-800 rounded-lg p-8 sm:p-12 bg-white mx-auto dark:bg-white dark:text-zinc-900 min-h-[250mm]">
        ...
        <div className="space-y-8 max-w-2xl mx-auto text-center flex-1">
          ...
          <p className="text-md text-zinc-800 leading-[1.8] font-medium mt-6 px-4">{getBodyText()}</p>
        ...
        <div className="flex justify-between items-end mt-auto pt-6 border-t border-amber-100">
          <div className="text-left">
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1">DATE OF ISSUE</p>
            <p className="text-sm font-bold text-zinc-800">{formatDate(cert.issueDate)}</p>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-xs font-bold text-zinc-900 uppercase tracking-wider">PRINCIPAL</p>
          </div>
        </div>
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <p className="text-sm font-bold text-zinc-900 truncate">{value || 'N/A'}</p>
    </div>
  );
}
